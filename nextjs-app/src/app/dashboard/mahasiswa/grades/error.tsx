...
export default function MahasiswaGradesError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error("Mahasiswa grades error:", error);
  }, [error]);
  ...
  return (
    <div className="min-h-[50vh] flex items-center justify-center p-4">
      <div className="bg-card-bg border border-border-color rounded-2xl shadow-sm p-8 max-w-md w-full text-center space-y-4">
        ...
        <div>
          <h2 className="text-xl font-bold text-text-primary">Gagal Memuat Rekap Nilai</h2>
          <p className="text-sm text-text-secondary mt-1">
            {error.message || "Terjadi kendala saat memuat hasil evaluasi penilaian."}
          </p>
        </div>
