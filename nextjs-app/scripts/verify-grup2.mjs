...
test("Poin 1: Rute /dashboard/mahasiswa/grades lengkap dengan page, loading, dan error", () => {
  assert(fs.existsSync(path.join(srcDir, "app/dashboard/mahasiswa/grades/page.tsx")), "page.tsx harus ada");
  assert(fs.existsSync(path.join(srcDir, "app/dashboard/mahasiswa/grades/loading.tsx")), "loading.tsx harus ada");
  assert(fs.existsSync(path.join(srcDir, "app/dashboard/mahasiswa/grades/error.tsx")), "error.tsx harus ada");
});
...
test("Poin 1: Sidebar Mahasiswa 'My Grades' terhubung ke /dashboard/mahasiswa/grades", () => {
  ...
  assert(layout.includes('{ label: "My Grades", href: "/dashboard/mahasiswa/grades", icon: "Award" }'), "My Grades harus terhubung ke rute nyata");
});
...
test("Poin 1: Logika filter nilai hanya mengembalikan data akun mahasiswa yang login (finalized only)", async () => {
  ...
  assert(Array.isArray(andiGrades) && andiGrades.length === 2, `Harus mengembalikan 2 nilai finalized untuk Andi, dapat: ${andiGrades.length}`);
  assert(andiGrades.every((g) => g.studentEmail === "mahasiswa@example.com"), "Seluruh data harus milik mahasiswa@example.com");
  assert(andiGrades.every((g) => g.status === "finalized"), "Hanya nilai finalized yang tampil untuk mahasiswa");
  
  // Pastikan data milik Siti Rahma tidak bocor ke Andi
  assert(!andiGrades.some((g) => g.studentEmail === "siti@example.com"), "Nilai milik mahasiswa lain tidak boleh bocor");
});

// 3. Verifikasi Poin 2: Feedback
test("Poin 2: Rute /dashboard/mahasiswa/feedback lengkap dengan page, loading, dan error", () => {
  assert(fs.existsSync(path.join(srcDir, "app/dashboard/mahasiswa/feedback/page.tsx")), "page.tsx harus ada");
  assert(fs.existsSync(path.join(srcDir, "app/dashboard/mahasiswa/feedback/loading.tsx")), "loading.tsx harus ada");
  assert(fs.existsSync(path.join(srcDir, "app/dashboard/mahasiswa/feedback/error.tsx")), "error.tsx harus ada");
});

test("Poin 2: Sidebar Mahasiswa 'Feedback' terhubung ke /dashboard/mahasiswa/feedback", () => {
  const layout = fs.readFileSync(path.join(srcDir, "app/dashboard/layout.tsx"), "utf-8");
  assert(layout.includes('{ label: "Feedback", href: "/dashboard/mahasiswa/feedback", icon: "MessageSquare" }'), "Feedback harus terhubung ke rute nyata");
});

test("Poin 2: Komponen feedback menampilkan komentar pengajar dan ulasan AI Copilot", () => {
  const content = fs.readFileSync(path.join(srcDir, "components/mahasiswa-feedback-full-section.tsx"), "utf-8");
...
test("Poin 3: Modal Submit Tugas (submit-assignment-modal.tsx) terpasang di dashboard mahasiswa", () => {
  ...
  assert(page.includes("<SubmitAssignmentButton />"), "Tombol SubmitAssignmentButton harus terpasang di header");
  assert(page.includes("<SubmitAssignmentModal />"), "Modal SubmitAssignmentModal harus terpasang di halaman");
});
...
test("Poin 3: API route /api/submissions memiliki validasi Zod dan proteksi role mahasiswa", () => {
  ...
  assert(route.includes('status: "submitted"'), "Harus memperbarui status assignment menjadi submitted");
});

test("Poin 3: Alur pengumpulan tugas mengubah status assignment menjadi 'submitted'", async () => {
  const { getAssignmentsStore, updateAssignmentInStore, getAssignmentByIdFromStore } = await import("../src/lib/assignmentStore.ts");
  ...
  assert(target && target.status === "pending", "Target tugas asg-002 harus berstatus pending sebelum disubmit");
