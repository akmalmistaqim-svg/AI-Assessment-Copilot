...
assert(newCls && newCls.id && newCls.name === 'Machine Learning', `Created class: ${newCls.name} (id: ${newCls.id})`);

const updatedCls = classStore.update(newCls.id, { name: 'Advanced Machine Learning' });
assert(updatedCls && updatedCls.name === 'Advanced Machine Learning', `Updated class: ${updatedCls?.name}`);
...
assert(newRubric && newRubric.id && newRubric.name === 'Rubrik Evaluasi Model AI', `Created rubric: ${newRubric.name} (id: ${newRubric.id})`);

const updatedRubric = rubricStore.update(newRubric.id, { criteriaCount: 5 });
assert(updatedRubric && updatedRubric.criteriaCount === 5, `Updated rubric criteria: ${updatedRubric?.criteriaCount}`);
...
assert(newAssignment && newAssignment.id && newAssignment.title === 'Implementasi Deep Learning PyTorch', `Created assignment: ${newAssignment.title} (id: ${newAssignment.id})`);

const updatedAssignment = assignmentStore.update(newAssignment.id, { title: 'Implementasi Transformer & PyTorch' });
assert(updatedAssignment && updatedAssignment.title === 'Implementasi Transformer & PyTorch', `Updated assignment: ${updatedAssignment?.title}`);
...
assert(newAssessment && newAssessment.id && newAssessment.name === 'Penilaian Proyek Akhir ML', `Created assessment: ${newAssessment.name} (id: ${newAssessment.id})`);

const updatedAssessment = assessmentStore.update(newAssessment.id, { status: 'in-review', gradedSubmissionsCount: 12 });
assert(updatedAssessment && updatedAssessment.status === 'in-review' && updatedAssessment.gradedSubmissionsCount === 12, `Updated assessment status & submissions`);
