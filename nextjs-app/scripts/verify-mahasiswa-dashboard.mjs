...
async function runTests() {
  ...
  assert(
    futureFeatureEls.length === 0,
    `Zero .js-future-feature elements on page (found: ${futureFeatureEls.length})`,
  );

  console.log("\n--- TEST GROUP 2: User Profile & Session Rendering ---");
  const userNameEls = document.querySelectorAll(".js-user-name");
  assert(
    userNameEls.length > 0 && userNameEls[0].textContent.includes("Andi Pratama"),
    'User name correctly populated as "Andi Pratama"',
  );
  const userAvatar = document.querySelector(".js-user-avatar");
  assert(
    userAvatar && userAvatar.textContent === "AP",
    'User initials avatar correctly rendered as "AP"',
  );
  ...
  assert(
    viewDashboard && !viewDashboard.classList.contains("hidden"),
    "Dashboard view is active by default",
  );
  ...
  assert(
    !viewAssignments.classList.contains("hidden") && viewDashboard.classList.contains("hidden"),
    "Navigated to My Assignments view (#view-assignments)",
  );
  ...
  linkGrades.click();
  assert(
    !viewGrades.classList.contains("hidden") && viewAssignments.classList.contains("hidden"),
    "Navigated to My Grades view (#view-grades)",
  );
  ...
  linkFeedback.click();
  assert(
    !viewFeedback.classList.contains("hidden") && viewGrades.classList.contains("hidden"),
    "Navigated to Feedback view (#view-feedback)",
  );
  ...
  linkSettings.click();
  assert(
    !viewSettings.classList.contains("hidden") && viewFeedback.classList.contains("hidden"),
    "Navigated to Settings view (#view-settings)",
  );
  ...
  linkHelp.click();
  assert(
    !viewHelp.classList.contains("hidden") && viewSettings.classList.contains("hidden"),
    "Navigated to Help view (#view-help)",
  );
  ...
  linkDashboard.click();
  assert(
    !viewDashboard.classList.contains("hidden") && viewHelp.classList.contains("hidden"),
    "Returned to Dashboard view (#view-dashboard)",
  );
  ...
  assert(
    fullAsgContainer.textContent.includes("Website CRUD"),
    'Pending filter displays pending assignment "Website CRUD"',
  );

  filterGradedBtn.click();
  assert(
    fullAsgContainer.textContent.includes("Database Design"),
    'Graded filter displays graded assignment "Database Design"',
  );
  ...
  assert(
    fullAsgContainer.textContent.includes("Website CRUD") &&
      fullAsgContainer.textContent.includes("Database Design"),
    "All filter displays all assignments",
  );
  ...
  assert(
    fullAsgContainer.textContent.includes("Website CRUD"),
    "Search input successfully filters assignments by keyword",
  );
  ...
  assert(
    ...
    "Submit Assignment modal opens on button click",
  );
  ...
  assert(
    submitModal.classList.contains("hidden"),
    "Submit Assignment modal closes after form submission",
  );

  const toastContainer = document.getElementById("toastContainer");
  assert(
    toastContainer.textContent.includes("Tugas berhasil dikumpulkan"),
    "Success toast notification displayed",
  );

  // Verify status in DOM
  assert(
    fullAsgContainer.textContent.includes("Terkumpul"),
    'Assignment item shows "Terkumpul" status badge in DOM',
  );
  ...
  assert(
    ...
    'Calendar modal opens on "Jadwal / Kalender" click',
  );
  ...
  assert(
    !gradeDetailModal.classList.contains("hidden"),
    'Grade detail modal opens on "Detail Rubrik" click',
  );
  assert(
    gradeDetailModal.textContent.includes("Arsitektur & Kualitas Kode"),
    "Rubric criteria loaded inside modal",
  );

  closeGradeModalBtn.click();
  assert(
    gradeDetailModal.classList.contains("hidden"),
    "Grade detail modal closes on close button click",
  );
  ...
  assert(
    notifBadgeDot.classList.contains("hidden"),
    'Unread notification badge dot cleared after "Tandai Dibaca"',
  );
  ...
  assert(
    updatedUser && updatedUser.name === "Andi Pratama Putra",
    'User name in localStorage updated to "Andi Pratama Putra"',
  );
